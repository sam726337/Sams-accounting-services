...
class RandomSalePanel(QWidget):
    ...
    def form_data(self) -> dict:
        ...
        if self.master_value_lookups:
            for field_name, label in (
                ...
                ("sale_ledger", "Sales Ledger"),
                ("cgst_ledger", "CGST Ledger"),
                ("sgst_ledger", "SGST Ledger"),
                ("round_off_ledger", "Round Off Ledger"),
            ):
                self.validate_master_value(values[field_name], "ledger", label)
    ...
    def load_tally_masters(self):
        ...
        self.master_value_lookups = {
            "ledger": {value.strip().casefold() for value in ledgers if value and value.strip()},
        ...
        self.set_status(
            "ok",
            f"Loaded {len(ledgers)} ledgers, {len(voucher_types)} voucher types, "
            f"{len(stock_items)} stock items. Type karke search karein aur list se select karein.",
        )
        self.set_busy(False)
