...
STOCK_ITEMS_XML = """<ENVELOPE>
  ...
  <BODY>
    <DESC>
      ...
      <TDL>
        <TDLMESSAGE>
          <COLLECTION NAME="Sams Stock Item Masters" ISMODIFY="No">
            <TYPE>StockItem</TYPE>
            <NATIVEMETHOD>Name</NATIVEMETHOD>
...
TALLY_GSTIN_PATTERN = re.compile(r"\b[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]\b", re.IGNORECASE)
...
LEDGERS_XML = """<ENVELOPE>
  ...
  <BODY>
    <DESC>
      ...
      <TDL>
        <TDLMESSAGE>
          <COLLECTION NAME="Sams Ledger Masters" ISMODIFY="No">
            <TYPE>Ledger</TYPE>
            <NATIVEMETHOD>Name</NATIVEMETHOD>
...
LEDGERS_GST_XML = """<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
    <TALLYREQUEST>EXPORT</TALLYREQUEST>
    <TYPE>COLLECTION</TYPE>
    <ID>Sams Ledger GST Masters</ID>
  </HEADER>
  <BODY>
    <DESC>
      <STATICVARIABLES>
      ...
      <TDL>
        <TDLMESSAGE>
          <COLLECTION NAME="Sams Ledger GST Masters" ISMODIFY="No">
            <TYPE>Ledger</TYPE>
            <NATIVEMETHOD>Name</NATIVEMETHOD>
            <NATIVEMETHOD>Parent</NATIVEMETHOD>
            <FETCH>Name,Parent,PartyGSTIN,GSTIN,GSTRegistrationType,LedGSTRegDetails.*</FETCH>
          </COLLECTION>
        </TDLMESSAGE>
      </TDL>
    </DESC>
...
def parse_tally_ledger_gst_records(raw_response: str) -> list[TallyLedgerGstRecord]:
    root = parse_tally_xml(raw_response)
    records: list[TallyLedgerGstRecord] = []
    seen: set[str] = set()

    for ledger in root.iter():
        if tag_name(ledger) != "LEDGER":
            continue

        name = clean_text(ledger.attrib.get("NAME"))
        parent = clean_text(ledger.attrib.get("PARENT"))
        gstin = ""
        registration_type = ""

        for child in ledger.iter():
            child_tag = tag_name(child)
            if child_tag == "NAME" and not name:
                name = clean_text(child.text)
            elif child_tag == "PARENT" and not parent:
                parent = clean_text(child.text)
            elif child_tag in {"PARTYGSTIN", "GSTIN", "GSTINUIN", "GSTINNUMBER", "BASICBUYERSSALESTAXNO"} and not gstin:
                gstin = normalize_tally_gstin(clean_text(child.text))
            elif child_tag == "GSTREGISTRATIONTYPE" and not registration_type:
                registration_type = clean_text(child.text)
...
def parse_stock_item_uom(raw_response: str) -> str:
    try:
        root = parse_tally_xml(raw_response)
    except ET.ParseError:
        return ""

    for wanted in ("BASEUNITS", "BASEUNITSNAME", "BASEUNIT"):
        for element in root.iter():
...
def fetch_tally_ledger_gst_records(tally_url: str, query: str = "") -> list[TallyLedgerGstRecord]:
    response = post_tally_xml(tally_url, LEDGERS_GST_XML)
    line_error = extract_line_error(response)
    if line_error:
        raise RuntimeError(line_error)
    records = parse_tally_ledger_gst_records(response)
    needle = query.strip().casefold()
    if needle:
        records = [record for record in records if needle in record.name.casefold()]
    return records
...
def update_tally_ledger_gstin(
    ...
    *,
    ...
    ledger = normalize_tally_text(ledger_name)
    normalized_gstin = normalize_tally_gstin(gstin)
    if not ledger:
        raise ValueError("Ledger name required hai.")
    if not is_valid_tally_gstin(normalized_gstin):
        raise ValueError(f"{ledger}: valid GSTIN required hai.")

    resolved_state = normalize_tally_text(state_name) or gst_state_name_from_gstin(normalized_gstin) or DEFAULT_PARTY_STATE
    xml = build_ledger_gstin_update_request(
        ledger_name=ledger,
...
def fetch_tally_stock_item_uom(tally_url: str, stock_item_name: str) -> str:
    stock_name = normalize_tally_text(stock_item_name)
    if not stock_name:
        return ""
    response = post_tally_xml(tally_url, build_stock_item_uom_request(stock_name))
    line_error = extract_line_error(response)
    if line_error:
        raise RuntimeError(line_error)
    return parse_stock_item_uom(response)


def build_stock_item_uom_request(stock_item_name: str) -> str:
    escaped_name = escape(stock_item_name)
    return (
        "<ENVELOPE>"
        "<HEADER>"
        "<VERSION>1</VERSION>"
...
def create_tally_item_invoice_voucher(
    ...
    *,
    ...
    cash_like_party = ledger_name_is_cash_like(party_ledger_name)
    party_state = DEFAULT_PARTY_STATE
    party_country = DEFAULT_PARTY_COUNTRY if cash_like_party else ""
    party_gst_registration_type = "Unregistered/Consumer" if cash_like_party else ""

    if party_ledger_name and not cash_like_party:
        try:
            party_state = fetch_tally_ledger_state(tally_url, party_ledger_name) or DEFAULT_PARTY_STATE
        except Exception:
            party_state = DEFAULT_PARTY_STATE

    resolved_uom = normalize_tally_text(item_uom)
    if item_rate is not None and billed_quantity is not None and not resolved_uom:
        try:
            resolved_uom = fetch_tally_stock_item_uom(tally_url, stock_item_name)
        except Exception:
            resolved_uom = ""
    ...
    xml = build_item_invoice_voucher_import_request(
        ...
        party_gst_registration_type=party_gst_registration_type,
...
def build_item_invoice_voucher_import_request(
    *,
    ...
    inventory_quantity_xml = ""
    inventory_rate_xml = ""
    if item_rate is not None and billed_quantity is not None:
        qty_text = format_tally_quantity(billed_quantity)
        safe_uom = normalize_tally_text(item_uom) or "Nos"
        inventory_quantity_xml = (
            f"<ACTUALQTY>{qty_text} {escape(safe_uom)}</ACTUALQTY>"
            f"<BILLEDQTY>{qty_text} {escape(safe_uom)}</BILLEDQTY>"
        )
        inventory_rate_xml = f"<RATE>{format_tally_amount(item_rate)}/{escape(safe_uom)}</RATE>"
    ...
    party_gst_registration_name = normalize_tally_text(party_gst_registration_type)
    party_gst_registration_xml = ""
    if party_gst_registration_name:
        party_gst_registration_xml = (
            f"<GSTREGISTRATIONTYPE>{escape(party_gst_registration_name)}</GSTREGISTRATIONTYPE>"
        )
    voucher_number_xml = (
    ...
    voucher_xml = (
        ...
        "<ISDEEMEDPOSITIVE>No</ISDEEMEDPOSITIVE>"
        f"{inventory_quantity_xml}"
        f"{inventory_rate_xml}"
...
def build_ledger_gstin_update_request(
    *,
    ledger_name: str,
    gstin: str,
    parent: str = "",
    ...
    safe_ledger = normalize_tally_text(ledger_name)
    safe_gstin = normalize_tally_gstin(gstin)
    safe_parent = normalize_tally_text(parent)
    safe_state = normalize_tally_text(state_name) or gst_state_name_from_gstin(safe_gstin) or DEFAULT_PARTY_STATE
    safe_registration = normalize_tally_text(registration_type) or "Regular"

    parent_xml = f"<PARENT>{escape(safe_parent)}</PARENT>" if safe_parent else ""
    state_xml = ""
    if safe_state:
        escaped_state = escape(safe_state)
        state_xml = (
            f"<LEDSTATENAME>{escaped_state}</LEDSTATENAME>"
            f"<PLACEOFSUPPLY>{escaped_state}</PLACEOFSUPPLY>"
        )

    ledger_xml = (
        f"<LEDGER NAME=\"{escape(safe_ledger)}\" ACTION=\"Alter\">"
        f"{parent_xml}"
        "<COUNTRYNAME>India</COUNTRYNAME>"
        f"{state_xml}"
        f"<GSTREGISTRATIONTYPE>{escape(safe_registration)}</GSTREGISTRATIONTYPE>"
        f"<PARTYGSTIN>{escape(safe_gstin)}</PARTYGSTIN>"
        "<LEDGSTREGDETAILS.LIST>"
        "<APPLICABLEFROM>20170701</APPLICABLEFROM>"
        f"<GSTREGISTRATIONTYPE>{escape(safe_registration)}</GSTREGISTRATIONTYPE>"
        f"<PLACEOFSUPPLY>{escape(safe_state)}</PLACEOFSUPPLY>"
        f"<GSTIN>{escape(safe_gstin)}</GSTIN>"
        "</LEDGSTREGDETAILS.LIST>"
        "</LEDGER>"
    )
    return build_master_import_request(master_xml=ledger_xml)


def build_master_import_request(*, master_xml: str) -> str:
    return (
        "<ENVELOPE>"
...
def normalize_tally_gstin(value: str | None) -> str:
    text = (value or "").strip().upper()
    match = TALLY_GSTIN_PATTERN.search(text)
    return match.group(0).upper() if match else text


def is_valid_tally_gstin(value: str | None) -> bool:
    return bool(TALLY_GSTIN_PATTERN.fullmatch((value or "").strip().upper()))


def gst_state_name_from_gstin(value: str | None) -> str:
    gstin = normalize_tally_gstin(value)
    return GST_STATE_NAMES.get(gstin[:2], "") if len(gstin) >= 2 else ""
...
def format_tally_quantity(value: Decimal) -> str:
    quantity = Decimal(value).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)
